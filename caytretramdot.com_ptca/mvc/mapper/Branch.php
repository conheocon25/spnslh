...
class Branch extends Mapper implements \MVC\Domain\BranchFinder {
    function __construct() {
        parent::__construct();				
        $this->selectAllStmt 	= self::$PDO->prepare("select * from branch order by name");
		...
		$this->findByPageStmt 	= self::$PDO->prepare("SELECT * FROM  branch order by name LIMIT :start,:max");		
		$this->findByGroupPageStmt 	= self::$PDO->prepare("SELECT * FROM  branch WHERE id_group=:id_group order by name LIMIT :start,:max");
    } 
