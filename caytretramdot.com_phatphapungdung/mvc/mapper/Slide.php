...
class Slide extends Mapper implements \MVC\Domain\SlideFinder {
    ...
    function __construct() {
		...
		$updateStmt 	= sprintf("update %s set id_presentation=?, name=?, `order`=?, note=?, `url`=? where id=?", $tblSlide);
		$insertStmt 	= sprintf("insert into %s ( id_presentation, name, `order`, note, `url`) values(?, ?, ?, ?, ?)", $tblSlide);
		$deleteStmt 	= sprintf("delete from %s where id=?", $tblSlide);
		$findByPageStmt = sprintf("SELECT * FROM  %s LIMIT :start,:max", $tblSlide);
		$findByStmt 	= sprintf("select * from %s where id_presentation=?", $tblSlide);
