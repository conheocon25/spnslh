...
class App extends Mapper implements \MVC\Domain\AppFinder {
    ...
    function __construct() {
        ...
        $this->selectAllStmt = self::$PDO->prepare( 
                            "select * from saigonlandhouse_app");
        $this->selectStmt = self::$PDO->prepare( 
                            "select * from saigonlandhouse_app where id=?");
		$this->findByStmt = self::$PDO->prepare( 
                            "select * from saigonlandhouse_app where alias=?");
		
		$this->notSignedStmt = self::$PDO->prepare("
							SELECT * 
							FROM `saigonlandhouse_app` 
							WHERE 
								id NOT IN ( 
        ...
        $this->updateStmt = self::$PDO->prepare( 
                            "update saigonlandhouse_app set 
								name=?, 
								phone=?, 
								address=?, 
								email=?, 
        ...
        $this->insertStmt = self::$PDO->prepare( 
                            "insert into saigonlandhouse_app ( name, phone, address, email, banner, prefix, app, datecreate, dateupdate, dateactivity, type) values(?,?,?,?,?,?,?,?,?,?,?)");
		$this->deleteStmt = self::$PDO->prepare( 
                            "delete from saigonlandhouse_app where id=?");
		$this->checkStmt = self::$PDO->prepare( 
                            "select distinct id from saigonlandhouse_app where app=?");			
		$this->checkEmailStmt = self::$PDO->prepare( 
                            "select distinct id from saigonlandhouse_app where email=?");									
