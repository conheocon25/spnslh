...
class Session extends Mapper implements \MVC\Domain\SessionFinder {
    function __construct(){
		...
		$updateStmt 			= sprintf("update %s set id_tracking=?, id_student=?, date_joined=?, count_month=?, state=?  where id=?", $tblSession);
		$insertStmt 			= sprintf("insert into %s ( id_tracking, id_student, date_joined, count_month, state) values(?, ?, ?, ?, ?)", $tblSession);
		$deleteStmt 			= sprintf("delete from %s where id=?", $tblSession);
		$deleteAllStmt 			= sprintf("DELETE FROM %s", $tblSession);
		...
		$findByTrackingTableStmt = sprintf("SELECT SE.id, SE.id_tracking, SE.id_student, SE.date_joined, SE.count_month, SE.state FROM %s SE INNER JOIN %s SU ON SE.id_student=SU.id WHERE SE.id_tracking=? AND SU.id_class=?", $tblSession, $tblStudent);
		$findByTrackingTableGenderStmt = sprintf("SELECT SE.id, SE.id_tracking, SE.id_student, SE.date_joined, SE.count_month, SE.state FROM %s SE INNER JOIN %s SU ON SE.id_student=SU.id WHERE SE.id_tracking=? AND SU.id_class=? AND SU.gender=?", $tblSession, $tblStudent);
		$findByTrackingGenderStmt= sprintf("SELECT SE.id, SE.id_tracking, SE.id_student, SE.date_joined, SE.count_month, SE.state FROM %s SE INNER JOIN %s SU ON SE.id_student=SU.id WHERE SE.id_tracking=? AND SU.gender=?", $tblSession, $tblStudent);
