...
class Store extends Mapper implements \MVC\Domain\StoreFinder {
    ...
    function __construct() {
		...
		$tblStore 		= "tbl_store";
		$selectAllStmt 	= sprintf("select * from %s", $tblStore);
		$selectStmt 	= sprintf("select *  from %s where id=?", $tblStore);
		$updateStmt 	= sprintf("update %s set introduction=?, count=? where id=?"	, $tblStore);
		$insertStmt 	= sprintf("insert into %s ( introduction, count) values(?, ?)"	, $tblStore);
